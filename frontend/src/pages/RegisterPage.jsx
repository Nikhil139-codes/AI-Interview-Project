...
import { register, reset, googleLoginAction } from '../features/auth/authSlice';
      ...
      const client = window.google.accounts.oauth2.initTokenClient({
        ...
        callback: (response) => {
          if (response && response.access_token) {
            ...
            setLocalError('Google signup was cancelled or failed.');
          }
        },
      });
      client.requestAccessToken();
    } catch (err) {
      setLocalError('Error initializing Google signup: ' + err.message);
    }
  };
      ...
      >
        <div className="glassmorphism rounded-3xl p-8 shadow-2xl border border-white/10">
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-gray-400 text-center font-medium">Start your journey to interview mastery.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full flex justify-center items-center gap-2 py-3 px-4 border border-transparent rounded-xl shadow-lg shadow-primary-500/25 text-sm font-bold text-white bg-gradient-to-r from-primary-600 to-primary-500 hover:from-primary-500 hover:to-primary-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 focus:ring-offset-dark-900 transition-all transform hover:scale-[1.02] mt-2 disabled:opacity-50"
            >
              {isLoading ? 'Signing Up...' : <>Sign Up <ArrowRight className="w-4 h-4" /></>}
            </button>
          </form>
          ...
          <div className="mt-8">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-dark-800 text-gray-400 font-medium rounded">Or sign up with</span>
              </div>
            </div>
